...
def prepare_args():
  ...
  parser.add_argument('--max_gpu_memory', type=float, default=0.95, help='Max gpu memory to use for the model')
...
def load_model(cache_configs):
  ...
  device_map = infer_auto_device_map(model, max_memory={0: "6GiB", 'cpu': "120GiB"})
  max_memory = get_max_memory()
  max_memory[0] = int(max_memory[0] * cache_configs['max_gpu_memory'])
  device_map = infer_auto_device_map(model, max_memory=max_memory)
  print(device_map)
  model = dispatch_model(model, device_map=device_map, main_device=0, offload_buffers=True)
