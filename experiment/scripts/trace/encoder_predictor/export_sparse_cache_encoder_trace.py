...
def validate_sparse_cache_switch_top1(config, model_path: Path) -> int:
    ...
    selected = sparse_cache_switch_selected_experts(config)
    if selected != 1:
        raise ValueError(
...
class SparseCacheTraceRunner:
    ...
    def load(self) -> None:
        ...
        validate_sparse_cache_switch_top1(config, self.args.model_path)
...
def _require_integer_tensor(split: str, name: str, tensor: torch.Tensor) -> None:
    if tensor.dtype not in {torch.int8, torch.int16, torch.int32, torch.int64, torch.uint8, torch.bool}:
        raise AssertionError(f"{split}: {name} must be an integer tensor, got {tensor.dtype}")
...
def _model_config_metadata(config) -> dict:
    return {
        ...
        "num_selected_experts": sparse_cache_switch_selected_experts(config),
        "num_sparse_encoder_layers": _required_int_config(config, "num_sparse_encoder_layers"),
        "num_sparse_decoder_layers": _required_int_config(config, "num_sparse_decoder_layers"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    validate_sparse_cache_switch_top1(model_config_for_dtype, args.model_path)
    ...
    writer = TraceWriter(output_dir, resolved_storage_dtype)
