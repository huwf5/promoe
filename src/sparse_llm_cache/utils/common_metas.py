...
class ModelMetas:
  ...
  def build_deepseek_v2_lite(return_dict = True):
    def parse_moe_layer_id(name):
      match = re.match(r'.*layers\.(\d+).*', name)
      if not match:
        return None
  ...
  @staticmethod
  def build_qwen2_57b_a14b(return_dict = True):
    def parse_moe_layer_id(name):
      match = re.match(r'.*layers\.(\d+).*', name)
      if not match:
        return None
...
predefined_metas = {
  ...
  'deepseek-ai/DeepSeek-V2-Lite-Chat'          : ModelMetas.build_deepseek_v2_lite,
  'Qwen/Qwen1.5-MoE-A2.7B-Chat'                : ModelMetas.build_qwen_moe,
  'Qwen/Qwen2-57B-A14B-Instruct'               : ModelMetas.build_qwen2_57b_a14b,
  'Qwen/Qwen2-57B-A14B-Instruct-GPTQ-Int4'     : ModelMetas.build_qwen2_57b_a14b,
