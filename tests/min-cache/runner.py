...
full_list.concat(template_cfg_list.copy()
  ...
  .override('cache_trace_path', ['/code/moe/moe-traces/min-cache-trace/deepseek-moe-chatgpt-prompt-small-0812'])
  .hyper_override(['cache_policy', 'num_predict_expert_per_layer', 'reorder_experts', 'early_preempt'], [
    # [0, False, False], ## weak baseline
    # [0, False,  True], ## storng baseline
    # ['min', 0,  True,  True], ## +opt
    # ['lru', 0,  True,  True], ## +opt
    # [6, False,  True], ## +p
    ['min', 6,  True,  True], ## +p+opt
    # ['lru', 6,  True,  True], ## +p+opt
  ])
)
# full_list.concat(template_cfg_list.copy()
