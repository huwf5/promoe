...
if cache_configs['cache_policy'] == 'min':
  from sparse_llm_cache.utils import RegexFilter, recursive_traverse_childrens
  gate_score_map = torch.load(f"{cache_configs['cache_trace_path']}/gate_score_map.pt")
  for gate_score_list in gate_score_map.values():
    for i in range(len(gate_score_list)):
      gate_score_list[i] = gate_score_list[i].to("cuda")
  gate_score_list = None

  next_idx = 0
  def replace_gate_report(model, filter):
    def f(module, name):
      def new_report_gate_scores(_):
        global next_idx
        ret = gate_score_list[next_idx]
        next_idx += 1
        return ret
      module.report_gate_scores = new_report_gate_scores
    recursive_traverse_childrens(model, f, filter)

  replace_gate_report(model, filter = RegexFilter(r'.*layers\.([1-9]\d*)\.mlp.gate$'))
...
for seq_id,text_list in enumerate(dl):
  ...
  if cache_configs['cache_policy'] == 'min':
    model._prefetch_mngr.cache.set_cur_seq(seq_id)
    gate_score_list = gate_score_map[seq_id]
    next_idx = 0
  input_len, output_len = gen_batch(text_list, max_new_tokens=len(gate_score_list)//model._prefetch_mngr.metas.num_layer, do_print=True)
